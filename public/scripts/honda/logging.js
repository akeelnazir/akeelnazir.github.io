/* ============================== sophus3 ==============================
   logging script for                                                               30/11/2012
   
   Honda - Honda UK
   
   Version 5.4.1ae-pd-huk_optin
...
//Active engagement with jQuery, mobile device check and cookie level check
...
try{
	...
	var showDomain = ["www.honda.co.uk", "uat.honda.co.uk", "localhost"],
	currentDomain = location.hostname,
	isValidDomain = showDomain.indexOf(currentDomain);

	var clevel=(Number(readCookie("HUK_OPTIN"))=="NaN")?4:Number(readCookie("HUK_OPTIN"));

	if ((clevel >= 2 || clevel == 0) && !isMobile.any() && screen.width>=1024  && isValidDomain !== -1){
		$('body').append("<div id=\"sophus3_tracking\" style=\"display:none\"><\/div>");
	}
}
catch(e){}
...
function s3_configured() {
	s3_tag_version = "5.4.1b";// no Plugin detection, see 521 comments
	s3_dtimeout = 5000;
...
function s3_get_log_URL(type,locn,time,displayed) {
	...
	if (type!="d") {
		...
		dg.huk_optin=readCookie("HUK_OPTIN")==null?4:readCookie("HUK_OPTIN");
...
if(!PluginDetect){
	var PluginDetect={
		...
		codebaseSearch:function(c){
			...
			var j=[0,0,0,0],g,f,b=e.plugin.digits,i=function(k,m){var l=(k==0?m:j[0])+","+(k==1?m:j[1])+","+(k==2?m:j[2])+","+(k==3?m:j[3]);return e.isActiveXObject(l)};
			var h,d,a=false;
...
function identify_plugins(){
	...
	if (navigator.plugins) {
		...
		for (i = 0; i < np.length; i++){
			plugins = "";
			plugins +="p:" + np[i].name;
			plugins +="+f:" +  np[i].filename;
			plugins +="+v:" +  np[i].version;
			plugins +=";"
			aPlugins[i]=plugins;
		}
	}
	if (aPlugins.length == 0) {
		...
		for ( p in pp ) {
			...
			if (version){
				plugins ="p:" + pp[p] + "+f:na+v:" + version + ";"
				aPlugins[p]=plugins;
			}
		}
		aPlugins.splice(aPlugins.length-1, 0,ieAcrobatVersion());
	}
	
	var retVal = "s3pi1=";
	var numExt =1;
	for (j in aPlugins){
		if ((retVal.length + aPlugins[j].length>400*numExt) && numExt<=5){
			numExt +=1;
			retVal += "&s3pi" + numExt + "="
		}
		retVal +=aPlugins[j];
	...
	if (window.ActiveXObject) {
		for (var x = 2; x < 10; x++) {
			try {
				...
				if (oAcro) 
				return "p:Adobe Acrobat+f:na+v:" + x + ".?;";
			} catch(ex) {}
		}
		try {
			oAcro4=new ActiveXObject('PDF.PdfCtrl.1');
			if (oAcro4)
			return "p:Adobe Acrobat+f:na+v:4.?;";
		} catch(ex) {}
		try {
			oAcro7=new ActiveXObject('AcroPDF.PDF.1');
			if (oAcro7)
			return "p:Adobe Acrobat+f:na+v:7.?;";
		} catch (ex) {}
		return "";
	}
}
